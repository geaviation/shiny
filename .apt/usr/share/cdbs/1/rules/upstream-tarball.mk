...
cdbs_python_exclude_check = $(if $(DEB_UPSTREAM_REPACKAGE_EXCLUDE),$(error ERROR: DEB_UPSTREAM_REPACKAGE_EXCLUDE no longer works - use find-style DEB_UPSTREAM_REPACKAGE_EXCLUDES instead))
...
DEB_UPSTREAM_RECEIVED_EXTENSION ?= $(if $(findstring $(DEB_UPSTREAM_TARBALL_EXTENSION),tgz),tar.gz,$(DEB_UPSTREAM_TARBALL_EXTENSION))
...
cdbs_upstream_tarball = $(cdbs_upstream_tarball_basename)$(DEB_UPSTREAM_TARBALL_DELIMITER)$(DEB_UPSTREAM_TARBALL_EXTENSION)
cdbs_upstream_received_tarball = $(DEB_SOURCE_PACKAGE)_$(DEB_UPSTREAM_TARBALL_VERSION).orig.$(DEB_UPSTREAM_RECEIVED_EXTENSION)
cdbs_upstream_local_basename = $(DEB_SOURCE_PACKAGE)_$(DEB_UPSTREAM_TARBALL_VERSION)$(if $(strip $(DEB_UPSTREAM_REPACKAGE_EXCLUDES)),$(DEB_UPSTREAM_REPACKAGE_DELIMITER)$(DEB_UPSTREAM_REPACKAGE_TAG))
cdbs_upstream_local_srcdir = $(cdbs_upstream_tarball_basename)$(if $(strip $(DEB_UPSTREAM_REPACKAGE_EXCLUDES)),$(DEB_UPSTREAM_REPACKAGE_DELIMITER)$(DEB_UPSTREAM_REPACKAGE_TAG))
cdbs_upstream_findargs_dirs = $(if $(filter %/,$(DEB_UPSTREAM_REPACKAGE_EXCLUDES)),-type d \( $(call cdbs_findargs-path-or-name,$(patsubst %/,%,$(filter %/,$(DEB_UPSTREAM_REPACKAGE_EXCLUDES)))) \))
cdbs_upstream_findargs_files = $(if $(filter-out %/,$(DEB_UPSTREAM_REPACKAGE_EXCLUDES)),-type f \( $(call cdbs_findargs-path-or-name,$(filter-out %/,$(DEB_UPSTREAM_REPACKAGE_EXCLUDES))) \))
...
get-orig-source:
	...
	@if [ ! -s "$(DEB_UPSTREAM_WORKDIR)/$(cdbs_upstream_received_tarball)" ] ; then \
		...
		wget $(DEB_UPSTREAM_WGET_OPTS) -O "$(DEB_UPSTREAM_WORKDIR)/$(cdbs_upstream_received_tarball)" "$(DEB_UPSTREAM_URL)/$(cdbs_upstream_tarball)" ; \
	else \
...
# recompressing (when $uncompress set and ...REPACKAGE_EXCLUDE unset)
	...
	if [ -n "$(strip $(DEB_UPSTREAM_REPACKAGE_EXCLUDES))" ] || [ "$$uncompress" = "false" ]; then \
		...
		if [ -n "$(cdbs_upstream_findargs_files)" ]; then \
			(set -e; cd "$(DEB_UPSTREAM_WORKDIR)/$(DEB_UPSTREAM_REPACKAGE_TAG)/$(DEB_UPSTREAM_TARBALL_SRCDIR)" && find $(cdbs_upstream_findargs_files) -delete); \
		fi && \
		if [ -n "$(cdbs_upstream_findargs_dirs)" ]; then \
			(set -e; cd "$(DEB_UPSTREAM_WORKDIR)/$(DEB_UPSTREAM_REPACKAGE_TAG)/$(DEB_UPSTREAM_TARBALL_SRCDIR)" && find $(cdbs_upstream_findargs_dirs) -exec rm -rf '{}' +;) \
		fi && \
		...
		GZIP="--best --no-name" tar -b1 --owner=root --group=root --mode=a+rX -czf "$(DEB_UPSTREAM_WORKDIR)/$(cdbs_upstream_local_basename).orig.tar.gz" -C "$(DEB_UPSTREAM_WORKDIR)/$(DEB_UPSTREAM_REPACKAGE_TAG)" "$(cdbs_upstream_local_srcdir)" && \
		echo "Cleaning up" && \
...
fail-source-not-repackaged:
	...
	@if find $(cdbs_upstream_findargs_files) $(and $(cdbs_upstream_findargs_files),$(cdbs_upstream_findargs_dirs),-o) $(cdbs_upstream_findargs_dirs) | grep '.*'; then \
		echo; \
		...
		echo '       (set DEB_UPSTREAM_REPACKAGE_EXCLUDES and use get-orig-source target)'; \
		exit 1; \
	fi
